...
class _BtnColor extends Gui.Button
{
    ...
    { 
        ...
        ON_NM_CUSTOMDRAW(gCtrl, lParam)
        {
            ...
            static PS_SOLID          := 0
            static hIconFont         := DllCall(
                "gdi32\CreateFontW",
                "int", -17, "int", 0, "int", 0, "int", 0,
                "int", 400, "uint", 0, "uint", 0, "uint", 0,
                "uint", 1, "uint", 0, "uint", 0, "uint", 5,
                "uint", 0, "str", "Segoe MDL2 Assets",
                "ptr"
            )
            ...
            ; Draw antialiased background -----------------------------------------------
            ;
            ; Do not clip the control with SetWindowRgn: a region has a hard pixel edge
            ; and would cut away GDI+'s antialiased transition pixels. Instead, paint the
            ; surrounding surface first and draw the rounded button on top.
            rc     := nmcd.rc
            corner := this._roundedCorner
            SetWindowRgn(gCtrl.hwnd, 0, 1)
            surfaceColor := this.HasProp("_surfaceColor")
                ? this._surfaceColor
                : _BtnColor.RgbToBgr("0x" this.Gui.BackColor)
            CB_DrawAntialiasedRoundRect(
                nmcd.hdc,
                rc.Width,
                rc.Height,
                corner,
            ...
            ; Draw text and icon ----------------------------------------------------------
            ;
            ; The control font is Segoe UI. Draw an optional MDL2 glyph separately,
            ; so Windows never has to use fallback metrics for the caption.
            SetBkMode(nmcd.hdc, 1)  ; TRANSPARENT
            SetTextColor(nmcd.hdc, this._textColor)

            hasIcon := this.HasProp("_iconGlyph") && this._iconGlyph != ""
            if hasIcon {
                CopyRect(textMeasure := !NMCUSTOMDRAWINFO.HasProp("RECT") && IsSet(RECT) ? RECT() : NMCUSTOMDRAWINFO.RECT(), nmcd.rc)
                DrawText(
                    nmcd.hdc,
                    StrPtr(gCtrl.Text),
                    -1,
                ...
                iconWidth := 17
                gap := 7
                totalWidth := iconWidth + gap + textMeasure.Width
                startX := Max(4, Floor((rc.Width - totalWidth) / 2))

                CopyRect(iconRect := !NMCUSTOMDRAWINFO.HasProp("RECT") && IsSet(RECT) ? RECT() : NMCUSTOMDRAWINFO.RECT(), nmcd.rc)
                iconRect.left := startX
                iconRect.right := startX + iconWidth
                oldFont := SelectObject(nmcd.hdc, hIconFont)
                DrawText(
                    nmcd.hdc,
                    StrPtr(this._iconGlyph),
                    -1,
                    iconRect,
                    DT_CENTER | DT_VCENTER | DT_SINGLELINE
                )
                SelectObject(nmcd.hdc, oldFont)

                CopyRect(textRect := !NMCUSTOMDRAWINFO.HasProp("RECT") && IsSet(RECT) ? RECT() : NMCUSTOMDRAWINFO.RECT(), nmcd.rc)
                textRect.left := iconRect.right + gap
                textRect.right := rc.right - 4
                textRect.top -= 1
                textRect.bottom += 2
                DrawText(
                    nmcd.hdc,
...
; ==============================================================================
; DocBot: antialiased rounded button background
; ==============================================================================

CB_DrawAntialiasedRoundRect(hdc, width, height, radius, fillBgr, surfaceBgr) {
    static token := CB_EnsureGdip()
    ...
    if DllCall("gdiplus\GdipCreateFromHDC", "ptr", hdc, "ptr*", &graphics) != 0
        return

    DllCall("gdiplus\GdipSetSmoothingMode", "ptr", graphics, "int", SmoothingModeAntiAlias)
    DllCall("gdiplus\GdipSetPixelOffsetMode", "ptr", graphics, "int", PixelOffsetModeHalf)
    DllCall("gdiplus\GdipGraphicsClear", "ptr", graphics, "uint", CB_BgrToArgb(surfaceBgr))

    diameter := Min(radius * 2.0, width - 1.0, height - 1.0)
    left := 0.5
    top := 0.5
    right := width - 0.5
    bottom := height - 0.5

    if DllCall("gdiplus\GdipCreatePath", "int", 0, "ptr*", &path) = 0 {
        DllCall("gdiplus\GdipAddPathArc", "ptr", path, "float", left, "float", top,
            "float", diameter, "float", diameter, "float", 180.0, "float", 90.0)
        DllCall("gdiplus\GdipAddPathArc", "ptr", path, "float", right - diameter, "float", top,
        ...
        DllCall("gdiplus\GdipAddPathArc", "ptr", path, "float", left, "float", bottom - diameter,
            "float", diameter, "float", diameter, "float", 90.0, "float", 90.0)
        DllCall("gdiplus\GdipClosePathFigure", "ptr", path)

        if DllCall("gdiplus\GdipCreateSolidFill", "uint", CB_BgrToArgb(fillBgr), "ptr*", &brush) = 0 {
...
CB_EnsureGdip() {
    ...
    if status != 0
        throw Error("GDI+ kon niet worden gestart (status " status ").")
    return token
}
