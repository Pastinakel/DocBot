...
These files ship **with the application**, not as user data — they are
replaced wholesale on every DocBot update. A migration here would only
become relevant if a future DocBot version needs to keep reading an older
bundled-package file format across an upgrade; that need does not exist
today.
